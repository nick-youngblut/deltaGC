...
=head1 USAGE
    ...
    grinder_readFiler.pl [options] > reads_filt.fasta
...
Reads must have unique read IDs as designated by grinder 
(eg. '>1' or '>2').

=head2 WARNING

Delete the read DB (*index file) if the script is killed before
...
my $outseq = Bio::SeqIO->new( -fh => \*STDOUT,
			      -format => 'fasta');
foreach my $seqo ( map { $read_db->get_Seq_by_id($_) }  keys %$keep_ids_r){
